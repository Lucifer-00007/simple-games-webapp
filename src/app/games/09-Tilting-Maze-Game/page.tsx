  ...
  useEffect(() => {
    const tiltingMazeGame = () => {
      ...
      function resetGame() {
        ...
        `;
        if (hardMode) {
          noteElement.innerHTML = `Click the joystick to start! <p>Hard mode, Avoid black holes. Back to easy mode? Press E</p>`;
        } else {
          noteElement.innerHTML = `Click the joystick to start! <p>Move every ball to the center. Ready for hard mode? Press H</p>`;
        }
      ...
      function main(timestamp) {
        ...
        try {
                  ...
                  if (ball.nextY + ballSize / 2 >= wall.y - wallW / 2 && ball.nextY - ballSize / 2 <= wall.y + wallW / 2) {
                    ...
                    if (ball.nextX + ballSize / 2 >= wallStart.x - wallW / 2 && ball.nextX < wallStart.x) {
                      ...
                      if (distance < ballSize / 2 + wallW / 2) {
                        ...
                        const rolled = rollAroundCap(wallStart, { x: closest.x, y: closest.y, velocityX: ball.velocityX, velocityY: ball.velocityY });
                    ...
                    if (ball.nextX - ballSize / 2 <= wallEnd.x + wallW / 2 && ball.nextX > wallEnd.x) {
                      ...
                      if (distance < ballSize / 2 + wallW / 2) {
                        ...
                        const rolled = rollAroundCap(wallEnd, { x: closest.x, y: closest.y, velocityX: ball.velocityX, velocityY: ball.velocityY });
                        Object.assign(ball, rolled);
                      }
                    }
          ...
          if (balls.every((ball) => distance2D(ball, { x: 350 / 2, y: 315 / 2 }) < 65 / 2)) {
            noteElement.innerHTML = `Congrats, you did it! ${!hardMode ? "<p>Press H for hard mode</p>" : ""} <p> Follow me <a href=\"https://twitter.com/HunorBorbely\" , target=\"_blank\">@HunorBorbely</a></p>`;
            noteElement.style.opacity = 1;
            gameInProgress = false;
          } else {
            previousTimestamp = timestamp;
          ...
          if (error.message == "The ball fell into a hole") {
            noteElement.innerHTML = `A ball fell into a black hole! Press space to reset the game. <p> Back to easy? Press E </p>`;
            noteElement.style.opacity = 1;
            gameInProgress = false;
          } else throw error;
        }
...
export default TiltingMazeGame;
